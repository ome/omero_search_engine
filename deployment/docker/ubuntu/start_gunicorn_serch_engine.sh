...
user=$USER
echo "Start Gunicorn ...."
#exec $PYTHONPATH/gunicorn "search_engine:create_app('production')" -b 0.0.0.0:5569 --timeout 0 --name "$NAME"   --bind=unix:$SOCKFILE  --log-file=$HOME/logs/engine_gunilog.log --access-logfile=$HOME/logs/engine_access.log -error-logfile=$HOME/engine_logs/error.log  --workers 2
