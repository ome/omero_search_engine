...
from omero_search_engine.validation.psql_templates import  tail_space_query,head_space_query, duplicated_keyvalue_pairs_query
import os
import pandas as pd
...
def prepare_the_sql_statment(sql_template, screen_name, project_name, add_where=""):
    if not screen_name and project_name:
        return sql_template.substitute(condition=" {add_where} project.name like '%{project_name}%'".format(add_where=add_where,project_name=project_name))
    elif not project_name and screen_name:
        return sql_template.substitute(condition=" {add_where} screen.name like '%{screen_name}%'".format(add_where=add_where,screen_name=screen_name))
...
def check_for_tail_space(screen_name,project_name):
    ...
    sql_statment=prepare_the_sql_statment(tail_space_query, screen_name, project_name," and")
...
def check_for_head_space(screen_name,project_name):
    ...
    sql_statment = prepare_the_sql_statment(head_space_query, screen_name, project_name," and")
...
def check_duplicated_keyvalue_pairs (screen_name, project_name):
    ...
    sql_statment = prepare_the_sql_statment(duplicated_keyvalue_pairs_query, screen_name, project_name, "where")
    duplicated_keyvalue_pairs_results = conn.execute_query(sql_statment)
    if len(duplicated_keyvalue_pairs_results)==0:
        search_omero_app.logger.info("No results is aviable for dupblicated keyvalue pairs ")
        return
    search_omero_app.logger.info("Generate reports for dupblicated keyvalue pairs ...")
    genrate_reports(duplicated_keyvalue_pairs_results, "dupblicated_keyvalue_pairs",screen_name,project_name)
...
def genrate_reports(results, check_type,screen_name,project_name):
    ...
    if (not screen_name and not project_name) or screen_name:
        df2=df.groupby(["screen_name", 'name', 'value']).size().reset_index().rename(columns={0: 'no of images'})
    ...
    if (not screen_name and not project_name) or project_name:
        df3 = df.groupby(["project_name", 'name', 'value']).size().reset_index().rename(columns={0: 'no of images'})
