...
bookmark, total_results=call_omero_return_results("%s%s" % (base_url, image_ext), data=query_data_json)

while len(recieved_results) < total_results:
    page += 1
    #add bookmark to the query, so it will return the next page
    query_data_ ={"query": {"query_details":{"and_filters": and_filters}}, "bookmark": bookmark}
    query_data_json_ = json.dumps(query_data_)
    #call the server
    bookmark,total_results=call_omero_return_results("%s%s" % (base_url, image_page_ext), data=query_data_json_)
