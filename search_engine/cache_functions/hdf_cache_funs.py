...
def write_key_values_h5py(resource_table, meta_values):
    cash_folder = search_omero_app.config["CASH_FOLDER"]
    cash_file_name = os.path.join(cash_folder, 'metadata_h5py.h5')
    f= h5py.File(cash_file_name, 'a')
    del_dataset=False
    try:
...
def delete_cashed_key(resource_table, key, value=None):
    cash_folder = search_omero_app.config["CASH_FOLDER"]
    cash_file_name = os.path.join(cash_folder, ".h5")
    f = h5py.File(cash_file_name, "a")
...
def check_cashed_query(resource_table, name, value, operator=None):
    '''
    ...
    cash_file_name = os.path.join(cash_folder, file_name)
    results=None
    if not os.path.exists(cash_file_name):
        return results
    try:
...
def cashe_query_results(resource_table, key_, results, operator):
    '''
