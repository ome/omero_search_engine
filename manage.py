...
@manager.option(
    "-r",
    "--resource",
    help="resource name, deleting all the indcese for all the resources is the default",
)
@manager.option(
    "-e",
    "--es_index",
...
def add_resource_data_to_es_index(resource=None, data_folder=None, from_json=False):
    ...
    if not resource or not data_folder or not os.path.exists(data_folder):
        search_omero_app.logger.info(
            "Please check the input parameters, resource: {resource}, data_folder: {data_folder}".format(
...
@manager.option(
    "-r",
    "--resource",
    help="resource name, creating all the indcese for all the resources is the default",
)
...
def set_database_configuration(
    ...
    if len(database_attrs) > 0:
        ...
        search_omero_app.logger.info(
            "At least one database attributed (i.e. url, database name, username, username password) should be provided"
        )
...
@manager.option(
    "-c", "--create_index", help="creating the elastic search index if set to True"
)
...
@manager.option(
    "-d",
    "--deep_check",
    help="compare all the images from both searhc engine and database server, default is False so it will compare the number of images and the first searchengine page",
)
